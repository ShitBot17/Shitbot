	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} -1 normie🤝`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} agora é admin então respeitem 🐊🤙`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} Acabou de perder o adm pressionem F!`, mentioned, true)
